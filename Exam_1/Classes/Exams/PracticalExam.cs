...
namespace Exam_1.Classes.Exams
{
    ...
    {
        public PracticalExam(int _examTime, int _numberOfQuestions, Question[] _questions, Subject _subject)
            : base(_examTime, _numberOfQuestions, _questions, _subject) 
        {
            for (int i = 0; i < _questions.Length; i++)
            {
                if (_questions[i] is not MCQQuestion)
                {
                    throw new ArgumentException("Practical Exam can contain MCQ questions only.");
                }
            }
        }
